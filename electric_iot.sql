...
CREATE TABLE IF NOT EXISTS `electric_iot`.`department` (
  `id` BIGINT NOT NULL,
  `uid` BIGINT NOT NULL DEFAULT 0 COMMENT '部门自定义ID',
  ...
  `is_delete` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '部门是否删除，1表示未删除，2表示删除',
  `modify_user_id` BIGINT NOT NULL COMMENT '修改人ID',
  `create_user_id` BIGINT NOT NULL COMMENT '创建人ID',
  `parent_id` BIGINT NOT NULL DEFAULT 0 COMMENT '父级组织的ID',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `FK_MODIFY_D_U_idx` (`modify_user_id` ASC),
  INDEX `FK_CREATE_D_U_idx` (`create_user_id` ASC),
  UNIQUE INDEX `uid_UNIQUE` (`uid` ASC),
  INDEX `FK_PARENT_D_D_idx` (`parent_id` ASC),
  CONSTRAINT `FK_MODIFY_D_U`
    FOREIGN KEY (`modify_user_id`)
    REFERENCES `electric_iot`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_CREATE_D_U`
  ...
  CONSTRAINT `FK_PARENT_D_D`
    ...
    REFERENCES `electric_iot`.`department` (`uid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '部门数据表';
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`right` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `uid` BIGINT NOT NULL DEFAULT 0 COMMENT '权限自定义ID',
  ...
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  UNIQUE INDEX `uid_UNIQUE` (`uid` ASC),
  INDEX `FK_PARENT_R_R_idx` (`parent_id` ASC),
  CONSTRAINT `FK_PARENT_R_R`
    FOREIGN KEY (`parent_id`)
    REFERENCES `electric_iot`.`right` (`uid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '权限数据表';
