...
CREATE TABLE IF NOT EXISTS `electric_iot`.`user` (
  ...
  `is_delete` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '用户是否删除，1表示未删除，2表示删除',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`department` (
  ...
  INDEX `FK_MODIFY_USER_idx` (`modify_user_id` ASC, `create_user_id` ASC),
  CONSTRAINT `FK_PARENT_D_D`
    FOREIGN KEY (`parent_id`)
    REFERENCES `electric_iot`.`department` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_MODIFY_CREATE_D_U`
    FOREIGN KEY (`modify_user_id` , `create_user_id`)
    REFERENCES `electric_iot`.`user` (`id` , `id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '部门数据表';
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`right` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `FK_PARENT_RIGHT_idx` (`parent_id` ASC),
  CONSTRAINT `FK_PARENT_R_R`
    FOREIGN KEY (`parent_id`)
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`status` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL COMMENT '设备状态名称',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC))
ENGINE = InnoDB
COMMENT = '设备状态数据表';
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device` (
  ...
  `serial_number` VARCHAR(200) NOT NULL COMMENT '设备序列号',
  `is_delete` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '设备是否删除，1表示未删除，2表示删除',
  `type_id` BIGINT NOT NULL COMMENT '设备类型ID',
  `status_id` BIGINT NOT NULL COMMENT '设备状态ID',
  `user_id` BIGINT NOT NULL COMMENT '设备所属的用户ID',
...
-- -----------------------------------------------------
-- Table `electric_iot`.`category`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `electric_iot`.`category` ;

CREATE TABLE IF NOT EXISTS `electric_iot`.`category` (
