...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `electric_iot`.`view_get_all_device_info` (`id` INT, `uid` INT, `imsi` INT, `name` INT, `serial_number` INT, `is_delete` INT, `type` INT, `status` INT, `user` INT, `department` INT, `comment` INT, `create_time` INT, `keep_live_interval` INT, `battery_sleep_time` INT, `battery_keep_live_time` INT);
...
SELECT
	...
	device.`comment`,
	device.create_time,
	device.keep_live_interval,
	device.battery_sleep_time,
	device.battery_keep_live_time 
FROM
	device
