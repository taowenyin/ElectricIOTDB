...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device` (
  ...
  `battery_send_gprs_interval` INT NOT NULL DEFAULT 180 COMMENT '电池供电时GPRS发送间隔，默认180分钟',
  `power_send_gprs_interval` INT NOT NULL DEFAULT 5 COMMENT '电源供电时GPRS发送间隔，默认5分钟',
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device_location` (
  ...
  `battery_voltage` INT NOT NULL COMMENT '记录设备电压',
  ...
  `nation_num` INT NULL COMMENT '设备的AGPS国家号',
  `mobile_num` INT NULL COMMENT '设备的AGPS移动网络号',
  `location_num` INT NULL COMMENT '设备的AGPS位置区域号',
  `community_num` VARCHAR(45) NULL COMMENT '设备的AGPS小区号',
  `station_num` VARCHAR(45) NULL COMMENT '设备的AGPS基站标识号',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device_cmd` (
  ...
  `set_gprs_send_interval` INT NULL COMMENT '当该位不为null是，表示需要设置设备的GPRS发送间隔时间',
  `get_gprs_send_interval` TINYINT(1) NULL COMMENT '当该位不为null是，表示需要获取设备的GPRS发送间隔时间',
