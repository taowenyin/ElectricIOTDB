...
CREATE TABLE IF NOT EXISTS `electric_iot`.`role_right_relation` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_ROLE_RELATION_idx` (`role_id` ASC) VISIBLE,
  INDEX `FK_RIGHT_RELATION_idx` (`right_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`user_right_relation` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_DEPARTMENT_RIGHT_RELATION_idx` (`right_id` ASC) VISIBLE,
  INDEX `FK_USER_RELATION_idx` (`user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_TYPE_DEVICE_idx` (`type_id` ASC) VISIBLE,
  INDEX `FK_STATUS_DEVICE_idx` (`status_id` ASC) VISIBLE,
  INDEX `FK_USER_D_U_idx` (`user_id` ASC) VISIBLE,
  INDEX `FK_DEPARTMENT_D_D_idx` (`department_id` ASC) VISIBLE,
  UNIQUE INDEX `imsi_UNIQUE` (`imsi` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device_location` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_DEVICE_LOCATION_idx` (`device_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`device_cmd` (
  ...
  `set_command` VARCHAR(45) NULL COMMENT '发送华电的通用指令',
  `is_send` TINYINT(1) NULL DEFAULT 0 COMMENT '该条指令是否已经发送，0表示未发送，1表示已发送',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_DEVICE_D_D_idx` (`device_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `electric_iot`.`bind_user_department_role` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_USER_BUDR_U_idx` (`user_id` ASC) VISIBLE,
  INDEX `FK_DEPARTMENT_BUDR_D_idx` (`department_id` ASC) VISIBLE,
  INDEX `FK_ROLE_BUDR_R_idx` (`role_id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `electric_iot`.`receive_device_data`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `electric_iot`.`receive_device_data` ;

CREATE TABLE IF NOT EXISTS `electric_iot`.`receive_device_data` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `receive_data` VARCHAR(45) NOT NULL COMMENT '接收到的设备数据',
  `device_id` BIGINT NOT NULL COMMENT '接收设备的ID',
  `is_read` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '数据是否已经使用',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `FK_RECEIVE_D_D_idx` (`device_id` ASC) VISIBLE,
  CONSTRAINT `FK_RECEIVE_D_D`
    FOREIGN KEY (`device_id`)
    REFERENCES `electric_iot`.`device` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
